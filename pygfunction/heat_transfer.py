...
def finite_line_source_approximation(
        time, alpha, dis, H1, D1, H2, D2, N=10, reaSource=True, imgSource=True):
    """
    Evaluate the Finite Line Source (FLS) solution.
    ...
    References
    ----------
    .. [#FLSApproxc-Cimmin2021] Cimmino, M. (2021). An approximation of the
    ...
    def exp1(x):
        ...
        return np.exp(-x)/(G + (1-G)*np.exp(-x/(1-G)))*np.log(1 + G/x - (1-G)/(h +b*x)**2)
    ...
    G1 = np.inner(p, q*np.inner(a, 0.5*exp1(np.expand_dims(dis**2, axis=(-1, -2)) + np.multiply.outer(q**2, b))))
    x3 = np.sqrt(np.expand_dims(dis**2, axis=-1) + q**2)
    G3 = np.inner(p, np.exp(-x3**2)/np.sqrt(pi) - x3*erfc(x3))
...
b_Q = [np.array([1.4751134849381495]),
...
a_erf = [np.array([1.] + [-2*a for a in a_Qi]) for a_Qi in a_Q]
b_erf = [np.array([0.] + [b*2 for b in b_Qi]) for b_Qi in b_Q]
